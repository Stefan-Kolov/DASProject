...
public class CsvService {
    private static final String CSV_FILE_PATH = "static/stock_data.csv";

    public List<StockData> readFromCSV() throws IOException {
        List<StockData> stockDataItems = new ArrayList<>();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd.M.yyyy");

        try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream(CSV_FILE_PATH);
             CSVReader reader = new CSVReader(new InputStreamReader(inputStream))) {
            if (inputStream == null) {
                throw new FileNotFoundException("CSV file not found in classpath: " + CSV_FILE_PATH);
            }
    ...
    */
    ...
    public List<String> getAvailableSymbols() throws IOException {
        List<StockData> marketItems = readFromCSV();
